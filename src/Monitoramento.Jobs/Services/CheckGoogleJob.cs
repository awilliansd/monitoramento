...
{
    public class CheckGoogleJob : IJobs
    {
        private readonly IServerHubManager _serverHubManager;
        private readonly UrlApiConfig _urlApiConfig;

        public CheckGoogleJob(IServerHubManager serverHubManager, UrlApiConfig urlApiConfig)
        {
        ...
        public async Task Check(string channelName, IJobCancellationToken cancellationToken)
        {
            ...
            try
            {
                var client = new RestClient(_urlApiConfig.UrlGoogle);
                ...
                {
                    _serverHubManager.Notify(EHubChannel.GoogleSite, EChannelCategory.Service,
                ...
                {
                    _serverHubManager.Notify(EHubChannel.GoogleSite, EChannelCategory.Service, channelName,
                        EStatusCode.Warning, $"Sucesso com {time.GetElapsedTimeBySecond()} segundos de atraso",
                        startTime);
                }
                else
                {
                    _serverHubManager.Notify(EHubChannel.GoogleSite, EChannelCategory.Service, channelName, EStatusCode.Ok,
